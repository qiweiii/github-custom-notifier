...
export function getWarningSymbol(warning: string): string {
  return warningSymbols.get(warning) || (warningSymbols.get('default') as string);
}

export function getErrorTitle(error: Error): string {
  return errorTitles.get(error.message) || (errorTitles.get('default') as string);
}

export function getErrorSymbol(error: Error): string {
  return errorSymbols.get(error.message) || (errorSymbols.get('default') as string);
}
